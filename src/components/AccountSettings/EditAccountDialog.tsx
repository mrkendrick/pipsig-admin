...
const EditAccountDialog = ({ open, onClose, market, account }: Props) => {
  const dispatch = useDispatch()
  const [loading, setLoading] = useState(false)

  const { _editAccount } = bindActionCreators(accountActions, dispatch)
  ...
  const submit: SubmitHandler<InputFields> = async data => {
    ...
    await _editAccount(account._id, newAccount, () => onClose(), setLoading)
  }

  const loadData = () => {
    setValue('symbol', account.symbol.toLowerCase())
    setValue('walletId', account.walletId)
  }
