...
const AddPlanDialog = ({ open, onClose }: Props) => {
  const [loading, setLoading] = useState(false)
  const dispatch = useDispatch()

  const { _addPlan } = bindActionCreators(plansActions, dispatch)
  ...
  const submit: SubmitHandler<InputFields> = async data => {
    setLoading(true)
    await _addPlan(data, () => onClose(), setLoading)
  }
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        ...
        <LoadingButton loading={loading} onClick={handleSubmit(submit)}>
          Add
        </LoadingButton>
      </DialogActions>
    </Dialog>
